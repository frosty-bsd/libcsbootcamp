...
SRC := src/csbootcamp.c
INCLUDE := src/csbootcamp.h
MANS := $(wildcard docs/*.3.gz)

CFLAGS=-Wall -Wextra -Werror -pedantic -std=c11
BASENAME=libcsbootcamp
...
deb: $(LIBS) $(MANS)
	...
	fpm \
	    ...
	    --description "csbootcamp library for C" \
	    --input-type dir \
	    --license "MIT" \
	    --maintainer "csbootcamp" \
	    --name $(BASENAME) \
	    --output-type deb \
	    --package build/deb \
	    --provides lib50-c \
	    ...
	    --url https://github.com/frosty-bsd/libcsbootcamp \
	    --vendor csbootcamp \
	    --version $(VERSION) \
	    .
...
.PHONY: uninstall
uninstall:
	rm -f $(DESTDIR)/include/csbootcamp.h
	rm -rf $(DESTDIR)/src/csbootcamp.c
